...
class SocksSocketImpl extends java.net.PlainSocketImpl implements java.net.SocksConsts {
    ...
    private java.net.Socket cmdsock = null;
    ...
    private int readSocksReply(InputStream in, byte[] data, long deadlineMillis) throws IOException {
        ...
        for (int attempts = 0; received < len && attempts < 3; attempts++) {
            ...
            if (count < 0)
                throw new java.net.SocketException("Malformed reply from SOCKS server");
            received += count;
        }
        return received;
    }
    ...
    private boolean authenticate(byte method, InputStream in,
        ...
        if (method == USER_PASSW) {
            ...
            final java.net.InetAddress addr = java.net.InetAddress.getByName(server);
...
//      }
    ...
    private void connectV4(InputStream in, OutputStream out,
                           ...
                           long deadlineMillis) throws IOException {
        if (!(endpoint.getAddress() instanceof java.net.Inet4Address)) {
            throw new java.net.SocketException("SOCKS V4 requires IPv4 only addresses");
        }
        ...
        if (n != 8)
            throw new java.net.SocketException("Reply from SOCKS server has bad length: " + n);
        if (data[0] != 0 && data[0] != 4)
            throw new java.net.SocketException("Reply from SOCKS server has bad version");
        java.net.SocketException ex = null;
        switch (data[1]) {
        case 90:
            // Success!
            ...
            break;
        case 91:
            ex = new java.net.SocketException("SOCKS request rejected");
            break;
        case 92:
            ex = new java.net.SocketException("SOCKS server couldn't reach destination");
            break;
        case 93:
            ex = new java.net.SocketException("SOCKS authentication failed");
            break;
        default:
            ex = new java.net.SocketException("Reply from SOCKS server contains bad status");
            break;
        }
        if (ex != null) {
            in.close();
    ...
    protected void connect(SocketAddress endpoint, int timeout) throws IOException {
        ...
        if (server == null) {
            ...
            java.net.ProxySelector sel = AccessController.doPrivileged(
                new PrivilegedAction<java.net.ProxySelector>() {
            ...
            // IPv6 litteral?
            if (epoint.getAddress() instanceof java.net.Inet6Address &&
                (!host.startsWith("[")) && (host.indexOf(":") >= 0)) {
                host = "[" + host + "]";
            }
            try {
                uri = new java.net.URI("socket://" + ParseUtil.encodePath(host) + ":"+ epoint.getPort());
            ...
            java.util.Iterator<java.net.Proxy> iProxy = null;
            ...
            while (iProxy.hasNext()) {
                p = iProxy.next();
                if (p == null || p.type() != java.net.Proxy.Type.SOCKS) {
                    super.connect(epoint, remainingMillis(deadlineMillis));
                    return;
                }

                if (!(p.address() instanceof InetSocketAddress))
                    throw new java.net.SocketException("Unknown address type for proxy: " + p);
                // Use getHostString() to avoid reverse lookups
             ...
             */
            if (server == null) {
                throw new java.net.SocketException("Can't connect to SOCKS proxy:"
                                          + savedExc.getMessage());
            }
        } else {
            // Connects to the SOCKS server
        ...
        if (((int)data[1]) == NO_METHODS)
            throw new java.net.SocketException("SOCKS : No acceptable methods");
        if (!authenticate(data[1], in, out, deadlineMillis)) {
            throw new java.net.SocketException("SOCKS : authentication failed");
        }
        out.write(PROTO_VERS);
        out.write(CONNECT);
        out.write(0);
        ...
        } else if (epoint.getAddress() instanceof java.net.Inet6Address) {
            out.write(IPV6);
        ...
        i = readSocksReply(in, data, deadlineMillis);
        if (i != 4)
            throw new java.net.SocketException("Reply from SOCKS server has bad length");
        java.net.SocketException ex = null;
        int len;
        byte[] addr;
        switch (data[1]) {
        case REQUEST_OK:
            ...
            case IPV4:
                ...
                if (i != 4)
                    throw new java.net.SocketException("Reply from SOCKS server badly formatted");
                data = new byte[2];
                i = readSocksReply(in, data, deadlineMillis);
                if (i != 2)
                    throw new java.net.SocketException("Reply from SOCKS server badly formatted");
            ...
            case IPV6:
                ...
                break;
            default:
                ex = new java.net.SocketException("Reply from SOCKS server contains wrong code");
                break;
            }
            break;
        case GENERAL_FAILURE:
            ex = new java.net.SocketException("SOCKS server general failure");
            break;
        case NOT_ALLOWED:
            ex = new java.net.SocketException("SOCKS: Connection not allowed by ruleset");
            break;
        case NET_UNREACHABLE:
            ex = new java.net.SocketException("SOCKS: Network unreachable");
            break;
        case HOST_UNREACHABLE:
            ex = new java.net.SocketException("SOCKS: Host unreachable");
            break;
        case CONN_REFUSED:
            ex = new java.net.SocketException("SOCKS: Connection refused");
            break;
        case TTL_EXPIRED:
            ex =  new java.net.SocketException("SOCKS: TTL expired");
            break;
        case CMD_NOT_SUPPORTED:
            ex = new java.net.SocketException("SOCKS: Command not supported");
            break;
        case ADDR_TYPE_NOT_SUP:
            ex = new java.net.SocketException("SOCKS: address type not supported");
            break;
        }
        if (ex != null) {
            in.close();
    ...
    private void bindV4(InputStream in, OutputStream out,
        ...
        if (!(baddr instanceof java.net.Inet4Address)) {
            throw new java.net.SocketException("SOCKS V4 requires IPv4 only addresses");
        }
        super.bind(baddr, lport);
        byte[] addr1 = baddr.getAddress();
        /* Test for AnyLocal */
        java.net.InetAddress naddr = baddr;
        if (naddr.isAnyLocalAddress()) {
            naddr = AccessController.doPrivileged(
                        new PrivilegedAction<java.net.InetAddress>() {
    ...
    protected synchronized void socksBind(InetSocketAddress saddr) throws IOException {
        ...
        if (server == null) {
            ...
            // IPv6 litteral?
            if (saddr.getAddress() instanceof java.net.Inet6Address &&
                (!host.startsWith("[")) && (host.indexOf(":") >= 0)) {
                host = "[" + host + "]";
            }
            try {
            ...
            while (iProxy.hasNext()) {
                ...
                try {
                    AccessController.doPrivileged(
                        new PrivilegedExceptionAction<Void>() {
                            public Void run() throws Exception {
                                cmdsock = new java.net.Socket(new java.net.PlainSocketImpl());
                    ...
                    // Ooops, let's notify the ProxySelector
                    sel.connectFailed(uri,p.address(),new java.net.SocketException(e.getMessage()));
                    server = null;
                    serverPort = -1;
                    cmdsock = null;
                    savedExc = e;
        ...
        if (saddr.isUnresolved()) {
        ...
        } else if (saddr.getAddress() instanceof java.net.Inet4Address) {
            ...
            out.write((lport >> 0) & 0xff);
            out.flush();
        } else if (saddr.getAddress() instanceof java.net.Inet6Address) {
            byte[] addr1 = saddr.getAddress().getAddress();
            out.write(IPV6);
            out.write(addr1);
            out.write((lport >> 8) & 0xff);
            ...
            cmdsock.close();
            throw new java.net.SocketException("unsupported address type : " + saddr);
        }
        data = new byte[4];
        ...
        case REQUEST_OK:
            ...
            case IPV4:
                ...
                external_address =
                    new InetSocketAddress(new java.net.Inet4Address("", addr) , nport);
                break;
            case DOMAIN_NAME:
                len = data[1];
                byte[] host = new byte[len];
            ...
            case IPV6:
                ...
                nport += ((int)data[1] & 0xff);
                external_address =
                    new InetSocketAddress(new java.net.Inet6Address("", addr), nport);
                break;
            }
            break;
        case GENERAL_FAILURE:
            ...
            break;
        case NOT_ALLOWED:
            ex = new java.net.SocketException("SOCKS: Bind not allowed by ruleset");
            break;
        case NET_UNREACHABLE:
            ex = new java.net.SocketException("SOCKS: Network unreachable");
            break;
    ...
    protected void acceptFrom(SocketImpl s, InetSocketAddress saddr) throws IOException {
        ...
        case GENERAL_FAILURE:
            ...
            break;
        case NOT_ALLOWED:
            ex = new java.net.SocketException("SOCKS: Accept not allowed by ruleset");
            break;
        case NET_UNREACHABLE:
            ex = new java.net.SocketException("SOCKS: Network unreachable");
            break;
