...
public class CookieManager extends CookieHandler
{
    ...
    private java.net.CookieStore cookieJar = null;
     ...
     *                          be used.
     */
    public CookieManager(java.net.CookieStore store,
                         java.net.CookiePolicy cookiePolicy)
    {
        // use default cookie policy if not specify one
        policyCallback = (cookiePolicy == null) ? java.net.CookiePolicy.ACCEPT_ORIGINAL_SERVER
                                                : cookiePolicy;
    ...
    public Map<String, List<String>>
        get(java.net.URI uri, Map<String, List<String>> requestHeaders)
        throws IOException
    {
        // pre-condition check
        ...
        List<java.net.HttpCookie> cookies = new java.util.ArrayList<java.net.HttpCookie>();
        String path = uri.getPath();
        if (path == null || path.isEmpty()) {
            path = "/";
        }
        for (java.net.HttpCookie cookie : cookieJar.get(uri)) {
    ...
    public void
        put(java.net.URI uri, Map<String, List<String>> responseHeaders)
        throws IOException
    {
        // pre-condition check
    ...
    private boolean shouldAcceptInternal(URI uri, java.net.HttpCookie cookie) {
        try {
     ...
     */
    private List<String> sortByPath(List<java.net.HttpCookie> cookies) {
    ...
    static class CookiePathComparator implements Comparator<java.net.HttpCookie> {
        public int compare(java.net.HttpCookie c1, HttpCookie c2) {
            if (c1 == c2) return 0;
            if (c1 == null) return -1;
            if (c2 == null) return 1;
