...
final class LambdaFormBuffer {
    ...
    LambdaFormBuffer(java.lang.invoke.LambdaForm lf) {
        this.arity = lf.arity;
        setNames(lf.names);
    ...
    private java.lang.invoke.LambdaForm lambdaForm() {
        assert(!inTrans());  // need endEdit call to tidy things up
        return new java.lang.invoke.LambdaForm(debugName, arity, nameArray(), resultIndex());
    }
