...
class Invokers {
    ...
    private final @java.lang.invoke.Stable
    ...
    private static LambdaForm callSiteForm(MethodType mtype, boolean skipCallSite) {
        ...
        final int LINKER_CALL  = nameCursor++;
        MethodType invokerFormType = mtype.appendParameterTypes(skipCallSite ? MethodHandle.class : java.lang.invoke.CallSite.class);
        Name[] names = arguments(nameCursor - INARG_LIMIT, invokerFormType);
        assert(names.length == nameCursor);
