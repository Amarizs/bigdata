...
class MutableBigInteger {
     ...
     */
    MutableBigInteger(java.math.BigInteger b) {
        intLen = b.mag.length;
    ...
    java.math.BigInteger toBigInteger(int sign) {
        ...
        return new java.math.BigInteger(getMagnitudeArray(), sign);
    }
     ...
     * and scale.
     */
    java.math.BigDecimal toBigDecimal(int sign, int scale) {
        if (intLen == 0 || sign == 0)
            return java.math.BigDecimal.zeroValueOf(scale);
        ...
        if (len > 2 || (d < 0 && len == 2))
            return new java.math.BigDecimal(new java.math.BigInteger(mag, sign), INFLATED, scale, 0);
        long v = (len == 2) ?
    ...
    void rightShift(int n) {
        ...
        if (nBits == 0)
            return;
        int bitsInHighWord = java.math.BigInteger.bitLengthForInt(value[offset]);
    ...
    private java.math.BigInteger getLower(int n) {
        if (isZero()) {
            ...
            while (len > 0 && value[offset+intLen-len] == 0)
                len--;
            int sign = len > 0 ? 1 : 0;
            return new java.math.BigInteger(Arrays.copyOfRange(value, offset+intLen-len, offset+intLen), sign);
        }
    }
    ...
    MutableBigInteger divide(MutableBigInteger b, MutableBigInteger quotient, boolean needRemainder) {
        if (b.intLen < java.math.BigInteger.BURNIKEL_ZIEGLER_THRESHOLD ||
                intLen - b.intLen < java.math.BigInteger.BURNIKEL_ZIEGLER_OFFSET) {
    ...
    MutableBigInteger divideAndRemainderBurnikelZiegler(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        if (r < s) {
            ...
            int m = 1 << (32-Integer.numberOfLeadingZeros(s/ java.math.BigInteger.BURNIKEL_ZIEGLER_THRESHOLD));
    ...
    private MutableBigInteger divide2n1n(MutableBigInteger b, MutableBigInteger quotient) {
        ...
        // step 1: base case
        if (n%2 != 0 || n < java.math.BigInteger.BURNIKEL_ZIEGLER_THRESHOLD) {
