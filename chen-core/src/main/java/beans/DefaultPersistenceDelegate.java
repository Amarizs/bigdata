...
public class DefaultPersistenceDelegate extends PersistenceDelegate {
    ...
    protected java.beans.Expression instantiate(Object oldInstance, java.beans.Encoder out) {
        ...
        return new java.beans.Expression(oldInstance, oldInstance.getClass(), "new", constructorArgs);
    }
    ...
    private Method findMethod(Class<?> type, String property) {
        ...
        java.beans.PropertyDescriptor pd = getPropertyDescriptor(type, property);
        if (pd == null) {
    ...
    private void doProperty(Class<?> type, java.beans.PropertyDescriptor pd, Object oldInstance, Object newInstance, java.beans.Encoder out) throws Exception {
        Method getter = pd.getReadMethod();
        Method setter = pd.getWriteMethod();

        if (getter != null && setter != null) {
            java.beans.Expression oldGetExp = new java.beans.Expression(oldInstance, getter.getName(), new Object[]{});
            java.beans.Expression newGetExp = new java.beans.Expression(newInstance, getter.getName(), new Object[]{});
                    ...
                    for(int i = 0; i < a.length; i = i + 3) {
                        try {
                           ...
                           if (f.get(null).equals(oldValue)) {
                               out.remove(oldValue);
                               out.writeExpression(new java.beans.Expression(oldValue, f, "get", new Object[]{null}));
                           }
                        }
                        catch (Exception ex) {}
                    }
    ...
    static void invokeStatement(Object instance, String methodName, Object[] args, java.beans.Encoder out) {
        out.writeStatement(new java.beans.Statement(instance, methodName, args));
    }

    // Write out the properties of this instance.
    private void initBean(Class<?> type, Object oldInstance, Object newInstance, java.beans.Encoder out) {
        for (Field field : type.getFields()) {
            ...
            try {
                java.beans.Expression oldGetExp = new java.beans.Expression(field, "get", new Object[] { oldInstance });
                java.beans.Expression newGetExp = new Expression(field, "get", new Object[] { newInstance });
                Object oldValue = oldGetExp.getValue();
                Object newValue = newGetExp.getValue();
                out.writeExpression(oldGetExp);
                if (!Objects.equals(newValue, out.get(oldValue))) {
        ...
        BeanInfo info;
        try {
            info = java.beans.Introspector.getBeanInfo(type);
        } catch (java.beans.IntrospectionException exception) {
            return;
        }
        // Properties
        for (java.beans.PropertyDescriptor d : info.getPropertyDescriptors()) {
            if (d.isTransient()) {
                continue;
            }
            try {
    ...
    private static java.beans.PropertyDescriptor getPropertyDescriptor(Class<?> type, String property) {
        try {
