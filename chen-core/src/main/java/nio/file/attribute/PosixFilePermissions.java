 ...
 * {@link java.nio.file.attribute.PosixFilePermission} objects.
 *
 * @since 1.7
 */
...
public final class PosixFilePermissions {
     ...
     */
    public static String toString(Set<java.nio.file.attribute.PosixFilePermission> perms) {
     ...
     * @see #toString(Set)
     */
    public static Set<java.nio.file.attribute.PosixFilePermission> fromString(String perms) {
        if (perms.length() != 9)
            throw new IllegalArgumentException("Invalid mode");
        Set<java.nio.file.attribute.PosixFilePermission> result = EnumSet.noneOf(java.nio.file.attribute.PosixFilePermission.class);
    ...
    /**
     * Creates a {@link java.nio.file.attribute.FileAttribute}, encapsulating a copy of the given file
     ...
     *          PosixFilePermission}
     */
    public static java.nio.file.attribute.FileAttribute<Set<java.nio.file.attribute.PosixFilePermission>>
        asFileAttribute(Set<java.nio.file.attribute.PosixFilePermission> perms)
    {
        // copy set and check for nulls (CCE will be thrown if an element is not
        // a PosixFilePermission)
        perms = new HashSet<java.nio.file.attribute.PosixFilePermission>(perms);
        for (java.nio.file.attribute.PosixFilePermission p: perms) {
            if (p == null)
                throw new NullPointerException();
        }
        final Set<java.nio.file.attribute.PosixFilePermission> value = perms;
        return new FileAttribute<Set<java.nio.file.attribute.PosixFilePermission>>() {
            @Override
            public String name() {
