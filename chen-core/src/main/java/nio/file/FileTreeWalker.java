...
class FileTreeWalker implements Closeable {
    ...
    private static class DirectoryNode {
        ...
        private final DirectoryStream<java.nio.file.Path> stream;
        private final Iterator<java.nio.file.Path> iterator;
        private boolean skipped;

        DirectoryNode(java.nio.file.Path dir, Object key, DirectoryStream<java.nio.file.Path> stream) {
            this.dir = dir;
            this.key = key;
            this.stream = stream;
    ...
    static class Event {
        ...
        private Event(EventType type, java.nio.file.Path file, BasicFileAttributes attrs, IOException ioe) {
            this.type = type;
            this.file = file;
            this.attrs = attrs;
            this.ioe = ioe;
        ...
        Event(EventType type, java.nio.file.Path file, BasicFileAttributes attrs) {
            this(type, file, attrs, null);
        }

        Event(EventType type, java.nio.file.Path file, IOException ioe) {
    ...
    FileTreeWalker(Collection<FileVisitOption> options, int maxDepth) {
        ...
        this.followLinks = fl;
        this.linkOptions = (fl) ? new java.nio.file.LinkOption[0] :
     ...
     */
    private BasicFileAttributes getAttributes(java.nio.file.Path file, boolean canUseCached)
        throws IOException
    {
        ...
        try {
            attrs = java.nio.file.Files.readAttributes(file, BasicFileAttributes.class, linkOptions);
        } catch (IOException ioe) {
            if (!followLinks)
                throw ioe;

            // attempt to get attrmptes without following links
            attrs = java.nio.file.Files.readAttributes(file,
     ...
     */
    private boolean wouldLoop(java.nio.file.Path dir, Object key) {
        ...
        for (DirectoryNode ancestor: stack) {
            ...
            if (key != null && ancestorKey != null) {
                ...
                try {
                    if (java.nio.file.Files.isSameFile(dir, ancestor.directory())) {
                        // cycle detected
                        return true;
                    }
     ...
     */
    private Event visit(java.nio.file.Path entry, boolean ignoreSecurityException, boolean canUseCached) {
     ...
     */
    Event walk(java.nio.file.Path file) {
        if (closed)
