...
class TempFileHelper {
    ...
    private static java.nio.file.Path generatePath(String prefix, String suffix, java.nio.file.Path dir) {
        long n = random.nextLong();
        n = (n == Long.MIN_VALUE) ? 0 : Math.abs(n);
        java.nio.file.Path name = dir.getFileSystem().getPath(prefix + Long.toString(n) + suffix);
     ...
     */
    private static java.nio.file.Path create(java.nio.file.Path dir,
                                             String prefix,
                                             String suffix,
        ...
        throws IOException
    {
        ...
        for (;;) {
            ...
            try {
                if (createDirectory) {
                    return java.nio.file.Files.createDirectory(f, attrs);
                } else {
     ...
     * temporary directory if dir is {@code null}.
     */
    static java.nio.file.Path createTempFile(java.nio.file.Path dir,
                                             String prefix,
                                             String suffix,
                                             FileAttribute<?>[] attrs)
        throws IOException
     ...
     * temporary directory if dir is {@code null}.
     */
    static java.nio.file.Path createTempDirectory(Path dir,
                                                  String prefix,
                                                  FileAttribute<?>[] attrs)
        throws IOException
    {
