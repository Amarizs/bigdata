 ...
 * the returned {@link java.nio.charset.CoderResult} object, or {@linkplain CodingErrorAction#REPLACE
...
public abstract class CharsetEncoder {
     ...
     */
    protected
    CharsetEncoder(java.nio.charset.Charset cs,
                   ...
                   byte[] replacement)
    {
        ...
        if (maxBytesPerChar <= 0.0f)
            throw new IllegalArgumentException("Non-positive "
                                               + "maxBytesPerChar");
        if (!java.nio.charset.Charset.atBugLevel("1.4")) {
     ...
     *          If the preconditions on the parameters do not hold
     */
    protected CharsetEncoder(java.nio.charset.Charset cs,
                             float averageBytesPerChar,
                             float maxBytesPerChar)
    {
        this(cs,
    ...
    private WeakReference<java.nio.charset.CharsetDecoder> cachedDecoder = null;
    ...
    public boolean isLegalReplacement(byte[] repl) {
        WeakReference<java.nio.charset.CharsetDecoder> wr = cachedDecoder;
        java.nio.charset.CharsetDecoder dec = null;
        ...
        CharBuffer cb = CharBuffer.allocate((int)(bb.remaining()
                                                  * dec.maxCharsPerByte()));
        java.nio.charset.CoderResult cr = dec.decode(bb, cb, true);
        return !cr.isError();
    }
     ...
     *   java.nio.charset.CoderResult#length() length} method.  This case applies only if the
     ...
     */
    public final java.nio.charset.CoderResult encode(CharBuffer in, ByteBuffer out,
                                                     boolean endOfInput)
    {
        ...
        for (;;) {
            ...
            if (cr.isUnderflow()) {
                if (endOfInput && in.hasRemaining()) {
                    cr = java.nio.charset.CoderResult.malformedForLength(in.remaining());
     ...
     * java.nio.charset.CoderResult#UNDERFLOW}.  If there is insufficient room in the output
     ...
     *         The output byte buffer
     *
     * @return  A coder-result object, either {@link java.nio.charset.CoderResult#UNDERFLOW} or
     *          {@link java.nio.charset.CoderResult#OVERFLOW}
     *
     * @throws  IllegalStateException
     ...
     *          parameter
     */
    public final java.nio.charset.CoderResult flush(ByteBuffer out) {
        if (state == ST_END) {
            java.nio.charset.CoderResult cr = implFlush(out);
            if (cr.isUnderflow())
                state = ST_FLUSHED;
            return cr;
        }
        ...
        return java.nio.charset.CoderResult.UNDERFLOW; // Already flushed
    }
     ...
     * returns {@link java.nio.charset.CoderResult#UNDERFLOW}.  This method should be overridden
     ...
     *          {@link java.nio.charset.CoderResult#OVERFLOW}
     */
    protected java.nio.charset.CoderResult implFlush(ByteBuffer out) {
     ...
     * limits will not be modified.
     *
     * <p> This method returns a {@link java.nio.charset.CoderResult} object to describe its
     ...
     * @return  A coder-result object describing the reason for termination
     */
    protected abstract java.nio.charset.CoderResult encodeLoop(CharBuffer in,
                                                               ByteBuffer out);
        ...
        throws CharacterCodingException
    {
        ...
        reset();
        for (;;) {
            java.nio.charset.CoderResult cr = in.hasRemaining() ?
