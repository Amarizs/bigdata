...
public abstract class PKIXRevocationChecker extends PKIXCertPathChecker {
    ...
    private List<java.security.cert.Extension> ocspExtensions = Collections.<java.security.cert.Extension>emptyList();
    private Map<java.security.cert.X509Certificate, byte[]> ocspResponses = Collections.emptyMap();
     ...
     */
    public void setOcspExtensions(List<java.security.cert.Extension> extensions)
    {
        this.ocspExtensions = (extensions == null)
                              ? Collections.<java.security.cert.Extension>emptyList()
                              : new ArrayList<java.security.cert.Extension>(extensions);
    }
     ...
     */
    public void setOcspResponses(Map<java.security.cert.X509Certificate, byte[]> responses)
    {
        if (responses == null) {
            this.ocspResponses = Collections.<java.security.cert.X509Certificate, byte[]>emptyMap();
        } else {
            Map<java.security.cert.X509Certificate, byte[]> copy = new HashMap<>(responses.size());
            for (Entry<java.security.cert.X509Certificate, byte[]> e : responses.entrySet()) {
     ...
     */
    public Map<java.security.cert.X509Certificate, byte[]> getOcspResponses() {
        Map<java.security.cert.X509Certificate, byte[]> copy = new HashMap<>(ocspResponses.size());
        for (Entry<java.security.cert.X509Certificate, byte[]> e : ocspResponses.entrySet()) {
            copy.put(e.getKey(), e.getValue().clone());
        }
        return copy;
    }
