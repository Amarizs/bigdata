...
implements Serializable
{
     ...
     */
    private PermissionCollection getPermissionCollection(java.security.Permission p,
                                                         boolean createEmpty) {
        Class<?> c = p.getClass();
     ...
     *
     */
    private PermissionCollection getUnresolvedPermissions(java.security.Permission p)
    {
        ...
        synchronized (unresolvedPerms) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                java.security.Permission perm = up.resolve(p, certs);
                if (perm != null) {
                    if (pc == null) {
                        pc = p.newPermissionCollection();
                        if (pc == null)
...
final class PermissionsEnumerator implements Enumeration<java.security.Permission> {

    // all the perms
    private Iterator<PermissionCollection> perms;
    // the current set
    private Enumeration<java.security.Permission> permset;
    ...
    private Enumeration<java.security.Permission> getNextEnumWithMore() {
        while (perms.hasNext()) {
            PermissionCollection pc = perms.next();
            Enumeration<java.security.Permission> next =pc.elements();
...
implements Serializable
{
     ...
     * Not serialized; see serialization section at end of class.
     */
    private transient Map<java.security.Permission, java.security.Permission> permsMap;
    ...
    PermissionsHash() {
        permsMap = new HashMap<java.security.Permission, java.security.Permission>(11);
    }
    ...
    public boolean implies(java.security.Permission permission) {
        ...
        synchronized (this) {
            java.security.Permission p = permsMap.get(permission);

            // If permission is found, then p.equals(permission)
            if (p == null) {
                for (java.security.Permission p_ : permsMap.values()) {
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        // Copy perms into a Hashtable
        Hashtable<java.security.Permission, java.security.Permission> perms =
    ...
    ClassNotFoundException {
        ...
        @SuppressWarnings("unchecked")
        Hashtable<java.security.Permission, java.security.Permission> perms =
                (Hashtable<java.security.Permission, java.security.Permission>)gfields.get("perms", null);
